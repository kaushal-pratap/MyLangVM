...
bool serialize(const Program *program, const char *filename){
    ...
    for(size_t index = 0; index < program->instruction_count; index++){
        VmInstruction vm_opcode = assembler_opcode_to_vm_opcode(program->instructions[index].opcode);

        if (fwrite(&vm_opcode,sizeof(vm_opcode),1,file) != 1) {
            fprintf(stderr, "Failed to write opcode !!\n");
            fclose(file);
            exit(EXIT_FAILURE);
        }
        if(program->instructions[index].has_operand){
            if (fwrite(&program->instructions[index].operand, sizeof(program->instructions[index].operand), 1, file) != 1) {
